...
import hdbscan
from models.photo import ClusterPhotoRep
from models.clustering_run import ClusteringRun

from models.photo import Photo,ClusterPhotoRepPhotoMapping
...
def get_rep_photos(session: Session, partner: str):
    ...
    try:
        ...
        rep_faces = session.execute(
            select(ClusterPhotoRep.run_id, ClusterPhotoRep.cluster_id, ClusterPhotoRep.rep_photo_id)
        ...
        return [{"run_id": row.run_id, "cluster_id": row.cluster_id, "rep_photo_id": row.rep_photo_id} for row in rep_faces]
...
def assign_representative_photo(photos, labels,partner):
    ...
    for label, photos_in_cluster in clustered_photos.items():
        ...
        representative_photos[label] = highest_score_photo.photo_id
...
def cluster_photos_for_partner(db: Session, partner: str):
    ...
    try:
        ...
        save_cluster_rep_photos(db, photos, labels, clustering, partner, clustering_run)

        # Step 5: Save the cluster-face mappings
        save_cluster_photo_mappings(db, photos, labels,clustering,clustering_run, partner)
...
def save_cluster_photo_mappings(db: Session, photos, labels, clustering, clustering_run, partner: str):
